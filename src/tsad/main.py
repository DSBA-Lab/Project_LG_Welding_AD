...
def main():
    ...
    # process init
    now = datetime.now()
    if args.subdataname is not None:
        group_name = f'{dataset}/{args.subdataname}-{model_name}'
    ...
    if args.log_to_wandb:
        wandb.init(
            ...
            project=project,
            config=config)
        logger.info('wandb init success!!')
    ...
    # model type init
    if model_name in ('LSTM_AE', 'LSTM_VAE', 'TF', 'USAD', 'DAGMM'):
    ...
    # load data
    trainloader, validloader, testloader = get_dataloader(data_name=args.dataname,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--project', type=str, default='tsad')
    ...
    parser.add_argument('--model', type=str, required=True,
                        choices=['LSTM_AE', 'LSTM_VAE', 'DAGMM', 'TF', 'USAD'])

    # data
    parser.add_argument('--dataname', type=str, required=True, help='data name(PSM)',
                        choices=['PSM', 'hamon'])
